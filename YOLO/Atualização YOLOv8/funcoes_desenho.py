"""
Funções auxiliares para implementação do YOLOv8 no Python
"""
import cv2
...
def label_caixa(image, box, label='', color=(128, 128, 128), txt_color=(255, 255, 255)):
  ...
  if label:
    ...
    w, h = cv2.getTextSize(label, 0, fontScale=lw / 3, thickness=tf)[0]  # largura e altura do texto
    outside = p1[1] - h >= 3
    p2 = p1[0] + w, p1[1] - h - 3 if outside else p1[1] + h + 3
    cv2.rectangle(image, p1, p2, color, -1, cv2.LINE_AA)  # retangulo preenchido (que ficará atrás do texto, para aumentar a legibilidade)
    cv2.putText(image,
...
def desenha_caixas(image, boxes, labels=[], colors=[], score=True, conf=None):
  ...
  # Define as cores (se quiser mudar as cores das caixas delimitadoras para cada classe, sao esses valores que voce deve mudar)
  if colors == []:
